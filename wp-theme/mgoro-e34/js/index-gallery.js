document.addEventListener("DOMContentLoaded", function () {
    ...
    // テーマディレクトリURLを取得（wp_localize_scriptで渡す or 自動検出）
    var themeUrl = (typeof mgoroVars !== "undefined" && mgoroVars.themeUrl)
        ? mgoroVars.themeUrl
        : (function () {
            var links = document.getElementsByTagName("link");
            for (var i = 0; i < links.length; i++) {
                var href = links[i].getAttribute("href") || "";
                if (href.indexOf("/wp-content/themes/mgoro-e34/style.css") !== -1) {
    ...
    var photosBase = themeUrl + "/photos/";
    var thumbsBase = themeUrl + "/photos/thumbs/";
    ...
    if (sliderPrimary && sliderSecondary && photos.length) {
        ...
        function setSlide(image, index) {
            image.src = photosBase + sliderPhotos[index];
            image.alt = "E34 535i slide " + (index + 1);
        }

        function preloadSlide(index, callback) {
            var loader = new Image();
            var source = photosBase + sliderPhotos[index];
    ...
    function renderGallery() {
        ...
        selected.forEach(function (filename, index) {
            ...
            item.href = photosBase + filename;
            item.target = "_blank";
            item.rel = "noopener noreferrer";

            image.src = thumbsBase + filename;
